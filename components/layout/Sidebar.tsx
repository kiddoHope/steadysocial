...
const NavItem: React.FC<NavItemProps> = ({ to, icon, label }) => (
  <NavLink
    to={to}
    className={({ isActive }) => ( // Explicit parentheses added around the template literal
...
const Sidebar: React.FC = () => {
  ...
  return (
    <aside className="w-64 bg-white dark:bg-slate-800 shadow-lg p-4 space-y-4 flex-shrink-0 h-full overflow-y-auto">
      <nav>
        <NavItem to="/dashboard" icon="fa-tachometer-alt" label="Dashboard" />
        {currentUser?.role === UserRole.CREATIVE && (
          <NavItem to="/generate" icon="fa-magic" label="Content Canvas" /> // Updated Label
        )}
